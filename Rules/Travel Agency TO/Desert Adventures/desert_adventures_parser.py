"""
Desert Adventures Parser
Extracts reservation data from Desert Adventures booking forms (PDF files or .msg files with PDF attachments)
"""
...
def extract_desert_adventures_fields(file_path):
    """
    Extract reservation fields from Desert Adventures booking form (.pdf or .msg file)
    ...
    try:
        ...
        # Extract first name and last name (Darina format: Names:YANYU CHEN)
        name_patterns = [
        ...
        for pattern in name_patterns:
            ...
            if match:
                ...
                if name_parts:
                    fields['MAIL_FIRST_NAME'] = name_parts[0]  # First name: YANYU
                    if len(name_parts) > 1:
                        fields['MAIL_LAST_NAME'] = ' '.join(name_parts[1:])  # Last name: CHEN
                break
        
        # Extract arrival date (Darina format: Check In:01-Oct-25)
        arrival_patterns = [
        ...
        # Extract departure date (Darina format: Check Out: 02-Oct-25)
        departure_patterns = [
        ...
        # Extract number of persons (Darina format: No. of Adult's:2)
        person_patterns = [
        ...
        # Extract room type (Darina format: Room Type:DBL / SUPERIOR ROOM / BB)
        room_patterns = [
        ...
        # Extract rate code or promo code (Darina format: Promotions:TOBBJN (On: 01-10))
        rate_patterns = [
...
def is_desert_adventures_file(file_path):
    """
    Check if file is from Desert Adventures
    
    Args:
        file_path (str): Path to the file (.pdf or .msg)
    
    Returns:
        bool: True if this is a Darina file
    """
    try:
        if file_path.lower().endswith('.msg'):
            msg = extract_msg.Message(file_path)
            ...
            return (
                'desert adventures' in content or
                'desert adventure' in content or
                'darina' in content or
                'booking form' in content or
                'reservation' in content
            )
        elif file_path.lower().endswith('.pdf'):
            with pdfplumber.open(file_path) as pdf:
                first_page_text = pdf.pages[0].extract_text() or ""
                return (
                    'desert adventures' in first_page_text.lower() or
                    'desert adventure' in first_page_text.lower() or
                    'darina' in first_page_text.lower() or
                    'booking form' in first_page_text.lower() or
                    'reservation' in first_page_text.lower()
                )
    except:
        return False
    return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        result = extract_desert_adventures_fields(file_path)
        
        print("Desert Adventures File Extraction Test:")
        print("=" * 50)
        ...
        print("Usage: python desert_adventures_parser.py <file_path>")