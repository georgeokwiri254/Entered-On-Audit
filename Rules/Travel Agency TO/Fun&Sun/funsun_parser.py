"""
Fun&Sun Parser
Extracts reservation data from Fun&Sun booking forms (PDF files or .msg files with PDF attachments)
"""
...
def extract_funsun_fields(file_path):
    """
    Extract reservation fields from Fun&Sun booking form (.pdf or .msg file)
    ...
    try:
        ...
        # Extract first name and last name (Fun&Sun format: Guest names or similar)
        name_patterns = [
        ...
        # Extract arrival date (Fun&Sun format: Check In or From date)
        arrival_patterns = [
        ...
        # Extract departure date (Fun&Sun format: Check Out or To date)
        departure_patterns = [
        ...
        # Extract number of persons (Fun&Sun format: No. of Adult's or similar)
        person_patterns = [
        ...
        # Extract room type (Fun&Sun format: Room Type or similar)
        room_patterns = [
        ...
        # Extract rate code or promo code (Fun&Sun format: Promotions or similar)
        rate_patterns = [
...
def is_funsun_file(file_path):
    """
    Check if file is from Fun&Sun
    
    Args:
        file_path (str): Path to the file (.pdf or .msg)
    
    Returns:
        bool: True if this is a Fun&Sun file
    """
    try:
        if file_path.lower().endswith('.msg'):
            msg = extract_msg.Message(file_path)
            ...
            return (
                'fun&sun' in content or
                'fun and sun' in content or
                'funsun' in content or
                'hotel reservation form' in content or
                'voucher' in content
            )
        elif file_path.lower().endswith('.pdf'):
            with pdfplumber.open(file_path) as pdf:
                ...
                return (
                    'fun&sun' in first_page_text.lower() or
                    'fun and sun' in first_page_text.lower() or
                    'funsun' in first_page_text.lower() or
                    'hotel reservation form' in first_page_text.lower() or
                    'voucher' in first_page_text.lower()
                )
    except:
        return False
    return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        result = extract_funsun_fields(file_path)
        
        print("Fun&Sun File Extraction Test:")
        print("=" * 50)
        ...
        print("Usage: python funsun_parser.py <file_path>")