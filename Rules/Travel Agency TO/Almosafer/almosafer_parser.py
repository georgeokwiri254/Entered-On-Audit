"""
Almosafer Email Parser
Extracts reservation data from Almosafer confirmation emails
"""

import re
from datetime import datetime

def extract_almosafer_fields(email_body, email_subject):
    """
    Extract reservation fields from Almosafer email content
    ...
    # Extract names - Almosafer specific mapping
    first_name_match = re.search(r'Name:\s*([A-Z\s]+)', email_body)
    last_name_match = re.search(r'Last Name:\s*([A-Z\s]+)', email_body)
    
    # For Almosafer: MAIL_FIRST_NAME = Name field, MAIL_FULL_NAME = Last Name field
    if first_name_match:
...
def is_almosafer_email(sender_email, subject):
    """
    Check if email is from Almosafer
    ...
    Returns:
        bool: True if this is a Almosafer email
    """
    return (
        'almosafer' in sender_email.lower() or 
        'almosafer' in subject.lower() or
        'confirmed booking' in subject.lower()
    )
...
if __name__ == "__main__":
    ...
    result = extract_almosafer_fields(sample_email, "Notice You received a new confirmed booking H2508221318877-1436342")
    
    print("Almosafer Extraction Test:")
    print("=" * 50)
